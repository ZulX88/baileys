# @zul-project/baileys - Typescript/Javascript WhatsApp Web API

## INFORMATION !! 
This Baileys has been modified by @zul-project. Don't use it for illegal activities or to harm other people. 
...
zul-project and its managers are not responsible for misuse of this application, as stated in the MIT license. zul-project management in no way condones the use of this application in practices that violate the WhatsApp Terms of Service. The manager of this application holds its users personally responsible for using this application in a fair manner, as it is intended to be used.
##
...
```
yarn add @zul-project/baileys
```

Use the edge version (no guarantee of stability, but latest fixes + features)
```
yarn add github:zul-project/baileys
```

Then import your code using:
``` ts 
import makeWASocket from '@zul-project/baileys'
```
...
``` ts
import makeWASocket, { DisconnectReason } from '@zul-project/baileys'
...
``` ts
import makeWASocket, { BufferJSON, ZulUseMultiFileAuthState } from '@zul-project/baileys'
import * as fs from 'fs'

// utility function to help save the auth state in a single folder
// this function serves as a good guide to help write auth & key states for SQL/no-SQL databases, which I would recommend in any production grade system
const { state, saveCreds } = await ZulUseMultiFileAuthState('auth_info_BaileysPro')
...
``` ts
import makeWASocket, { makeInMemoryStore } from '@zul-project/baileys'
...
``` ts
import { MessageType, MessageOptions, Mimetype } from '@zul-project/baileys'
...
- **MiscGenerationOptions**: some extra info about the message. It can have the following __optional__ values:
    ...
    const info: MessageOptions = {
        ...
        mimetype: Mimetype.pdf, /* (for media messages) specify the type of media (optional for all media types except documents),
                                    import {Mimetype} from '@zul-project/baileys'
                                */
...
import { downloadMediaMessage } from '@zul-project/baileys'
...
## Sending All Interactive Buttons:
 ```
const { generateWAMessageFromContent, proto } = require("@zul-project/baileys")
